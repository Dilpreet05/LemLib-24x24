...
void spinIntake(){
	...
	if(master.get_digital(pros::E_CONTROLLER_DIGITAL_L1)){ // Move wheel intake while holding L1
		flexWheelIntakeMotor.move(127);
	}else{ // Stop wheel intake otherwise
		flexWheelIntakeMotor.move(0);
		flexWheelIntakeMotor.brake();
	}

	if(master.get_digital(pros::E_CONTROLLER_DIGITAL_R1)){ // Move hook intake while holding R1
		hookIntakeMotor.move(127);
	}else if(master.get_digital(pros::E_CONTROLLER_DIGITAL_L2)){ // Move hook intake the other way while holding L2
		hookIntakeMotor.move(-127);
	}else{ // Stop otherwise
...
void setIntakeBrakes(){ // Set the intake motors to break with coast
